...
from aura.core.paths.global_paths import AURA_HOME, GlobalPath
from aura.core.trusted_folders import trusted_folders_manager
...
def _resolve_config_path(basename: str, type: Literal["file", "dir"]) -> Path:
    ...
    if not is_folder_trusted:
        return AURA_HOME.path / basename
    if type == "file":
        if (candidate := cwd / ".aura" / basename).is_file():
            return candidate
    elif type == "dir":
        if (candidate := cwd / ".aura" / basename).is_dir():
            return candidate
    return AURA_HOME.path / basename
...
def resolve_local_tools_dir(dir: Path) -> Path | None:
    if not trusted_folders_manager.is_trusted(dir):
        return None
    if (candidate := dir / ".aura" / "tools").is_dir():
        return candidate
    return None
...
def resolve_local_skills_dir(dir: Path) -> Path | None:
    if not trusted_folders_manager.is_trusted(dir):
        return None
    if (candidate := dir / ".aura" / "skills").is_dir():
        return candidate
    return None
...
def resolve_local_agents_dir(dir: Path) -> Path | None:
    if not trusted_folders_manager.is_trusted(dir):
        return None
    if (candidate := dir / ".aura" / "agents").is_dir():
        return candidate
    return None
...
HISTORY_FILE = ConfigPath(lambda: _resolve_config_path("aurahistory", "file"))
