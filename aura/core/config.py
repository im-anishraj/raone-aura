...
from aura.core.paths.config_paths import CONFIG_DIR, CONFIG_FILE, PROMPTS_DIR
from aura.core.paths.global_paths import (
    GLOBAL_ENV_FILE,
    GLOBAL_PROMPTS_DIR,
    SESSION_LOG_DIR,
)
from aura.core.prompts import SystemPrompt
from aura.core.tools.base import BaseToolConfig
...
DEFAULT_PROVIDERS = [
    ...
    ProviderConfig(
        ...
        api_key_env_var="",  # Ollama needs no API key
...
class AuraConfig(BaseSettings):
    ...
    system_prompt_id: str = "terminal_mind"
    ...
    # TODO(aura-nuage): remove exclude=True once the feature is publicly available
    ...
    # TODO(aura-nuage): change default value to MISTRAL_API_KEY once prod has shared aura-nuage workers
    ...
    def _check_api_key(self) -> AuraConfig:
        try:
    ...
    @model_validator(mode="after")
    def _check_api_backend_compatibility(self) -> AuraConfig:
        try:
            active_model = self.get_active_model()
            provider = self.get_provider_for_model(active_model)
            MISTRAL_API_BASES = [
            ...
            if (is_mistral_api and provider.backend != Backend.MISTRAL) or (
                not is_mistral_api and provider.backend not in (Backend.GENERIC, Backend.OPENAI, Backend.OLLAMA)
            ):
    ...
    @model_validator(mode="after")
    def _validate_model_uniqueness(self) -> AuraConfig:
    ...
    @model_validator(mode="after")
    def _check_system_prompt(self) -> AuraConfig:
    ...
    @classmethod
    def load(cls, **overrides: Any) -> AuraConfig:
        cls._migrate()
