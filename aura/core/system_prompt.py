...
from aura.core.trusted_folders import TRUSTABLE_FILENAMES, trusted_folders_manager
from aura.core.utils import is_dangerous_directory, is_windows

if TYPE_CHECKING:
    from aura.core.agents import AgentManager
    from aura.core.config import ProjectContextConfig, AuraConfig
...
def _add_commit_signature() -> str:
    return (
        "When committing changes, use 'git commit'.\n"
        "Always suffix your commit with the following heredoc:\n\n"
        "```bash\n"
        "git commit -m <Commit message here>\n\n"
        "Executed by RaOne Aura.\n"
        "Co-Authored-By: RaOne Aura <system@raone.dev>\n"
        "```"
    )
...
def _get_available_skills_section(skill_manager: SkillManager) -> str:
    ...
    lines = [
        "# Available Protocols",
        "",
        "You have access to the following protocols. When a task matches a protocol's description,",
    ...
    lines.append("</available_protocols>")
...
def _get_available_subagents_section(agent_manager: AgentManager) -> str:
    ...
    lines = ["# Available Cybots (Subagents)", ""]
    lines.append("The following specialized Cybots can be spawned via the Task tool:")
    for agent in agents:
