...
from aura.core.paths.config_paths import resolve_local_tools_dir
from aura.core.paths.global_paths import DEFAULT_TOOL_DIR, GLOBAL_TOOLS_DIR
from aura.core.tools.base import BaseTool, BaseToolConfig
from aura.core.tools.mcp import (
    RemoteTool,
...
from aura.core.utils import name_matches, run_sync

logger = getLogger("aura")

if TYPE_CHECKING:
    from aura.core.config import MCPHttp, MCPStdio, MCPStreamableHttp, AuraConfig


def _try_canonical_module_name(path: Path) -> str | None:
    """Extract canonical module name for aura package files.
...
def _compute_module_name(path: Path) -> str:
    """Return canonical module name for aura files, hash-based synthetic name otherwise."""
...
class ToolManager:
    ...
    def __init__(self, config_getter: Callable[[], AuraConfig]) -> None:
    ...
    @property
    def _config(self) -> AuraConfig:
        return self._config_getter()

    @staticmethod
    def _compute_search_paths(config: AuraConfig) -> list[Path]:
