...
from aura.cli.textual_ui.app import run_textual_ui
...
from aura.core.paths.config_paths import CONFIG_FILE, HISTORY_FILE
from aura.core.programmatic import run_programmatic
...
from aura.setup.onboarding import run_onboarding
...
def load_config_or_exit() -> AuraConfig:
    try:
        return AuraConfig.load()
...
def bootstrap_config_files() -> None:
    if not CONFIG_FILE.path.exists():
        try:
            AuraConfig.save_updates(AuraConfig.create_default())
    ...
    if not HISTORY_FILE.path.exists():
        try:
            ...
            HISTORY_FILE.path.write_text("Hello Aura!\n", "utf-8")
        except Exception as e:
            rprint(f"[yellow]Could not create history file: {e}[/]")


def load_session(
    args: argparse.Namespace, config: AuraConfig
