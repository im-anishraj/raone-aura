...
from aura.setup.onboarding.base import OnboardingScreen

PROVIDER_HELP = {
    "mistral": ("https://console.mistral.ai/codestral/cli", "Mistral AI Studio")
}
CONFIG_DOCS_URL = (
    "https://github.com/mistralai/mistral-aura?tab=readme-ov-file#configuration"
)
...
class ApiKeyScreen(OnboardingScreen):
    ...
    def __init__(self) -> None:
        super().__init__()
        config = AuraConfig.model_construct()
    ...
    def _compose_config_docs(self) -> ComposeResult:
        yield Static("[dim]Learn more about Aura configuration:[/]")
        yield Horizontal(
            NoMarkupStatic("→ ", classes="link-chevron"),
