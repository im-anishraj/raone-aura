...
from aura import AURA_ROOT, __version__
from aura.acp.tools.base import BaseAcpTool
from aura.acp.tools.session_update import (
    tool_call_session_update,
    tool_result_session_update,
)
from aura.acp.utils import (
    TOOL_OPTIONS,
    ToolOption,
...
from aura.core.config import MissingAPIKeyError, AuraConfig, load_dotenv_values
...
class VibeAcpAgentLoop(AcpAgent):
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        #  - dev mode: `uv run aura-acp`, ran from the project root
        #  - uv tool install: `aura-acp`, similar to dev mode, but uv takes care of path resolution
        #  - bundled binary: `./aura-acp` from binary location
        ...
        # So sys.executable is already /path/to/binary/aura-acp.
        ...
        auth_methods = (
            [
                AuthMethod(
                    ...
                    description="Register your API Key inside Mistral Aura",
                    field_meta={
                        "terminal-auth": {
                            "command": command,
                            "args": args,
                            "label": "Mistral Aura Setup",
                        }
                    },
                )
            ]
    ...
    async def new_session(
        ...
        **kwargs: Any,
        ...
        try:
            config = AuraConfig.load(disabled_tools=["ask_user_question"])
    ...
    def _get_acp_tool_overrides(self) -> list[Path]:
        ...
        return [
            AURA_ROOT / "acp" / "tools" / "builtins" / f"{override}.py"
    ...
    async def set_session_model(
        ...
        AuraConfig.save_updates({"active_model": model_id})

        new_config = AuraConfig.load(
