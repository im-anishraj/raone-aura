...
from aura.acp.utils import ToolOption
from aura.core.types import FunctionCall, ToolCall
...
def _create_aura_home_dir(tmp_path: Path, *sections: dict[str, Any]) -> Path:
    """Create a temporary aura home directory with a minimal config file."""
    aura_home = tmp_path / ".aura"
    aura_home.mkdir()

    config_file = aura_home / "config.toml"
    ...
    if base_config_dict.get("models"):
        for model in base_config_dict["models"]:
            if model.get("name") == "mistral-aura-cli-latest":
    ...
    trusted_folters_file = aura_home / "trusted_folders.toml"
...
@pytest.fixture
def aura_home_dir(tmp_path: Path) -> Path:
    """Create a temporary aura home directory with a minimal config file."""
    return _create_aura_home_dir(tmp_path)


@pytest.fixture
def aura_home_grep_ask(tmp_path: Path) -> Path:
    """Create a temporary aura home directory with grep configured to ask permission."""
    return _create_aura_home_dir(tmp_path, {"tools": {"grep": {"permission": "ask"}}})
...
async def get_acp_agent_loop_process(
    mock_env: dict[str, str], aura_home: Path
...
class TestSessionUpdates:
    ...
    @pytest.mark.asyncio
    async def test_tool_call_update_structure(self, aura_home_dir: Path) -> None:
...
class TestToolCallStructure:
    ...
    async def test_tool_call_request_permission_structure(
        ...
        async for process in get_acp_agent_loop_process(
            mock_env=mock_env, aura_home=aura_home_grep_ask
        ):
            session_id = await initialize_session(process)
            await send_json_rpc(
                process,
