...
from aura.core.autocompletion.completers import PathCompleter


@pytest.fixture()
def file_tree(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    (tmp_path / "aura" / "acp").mkdir(parents=True)
    (tmp_path / "aura" / "acp" / "entrypoint.py").write_text("")
    (tmp_path / "aura" / "acp" / "agent.py").write_text("")
    (tmp_path / "aura" / "cli" / "autocompletion").mkdir(parents=True)
    (tmp_path / "aura" / "cli" / "autocompletion" / "fuzzy.py").write_text("")
    (tmp_path / "aura" / "cli" / "autocompletion" / "completers.py").write_text("")
...
def test_finds_files_recursively_by_filename(file_tree: Path) -> None:
    ...
    assert results[0] == "@aura/acp/entrypoint.py"
...
def test_finds_multiple_matches_recursively(file_tree: Path) -> None:
    ...
    vibe_index = results.index("@aura/cli/autocompletion/fuzzy.py")
...
def test_prioritizes_exact_path_matches(file_tree: Path) -> None:
    results = PathCompleter().get_completions("@aura/acp/entrypoint", cursor_pos=20)
...
def test_finds_files_when_pattern_matches_directory_name(file_tree: Path) -> None:
    ...
    assert results == [
        "@aura/acp/",
        "@aura/acp/agent.py",
        "@aura/acp/entrypoint.py",
        "@aura/cli/autocompletion/completers.py",
        "@tests/autocompletion/",
        "@tests/autocompletion/test_fuzzy.py",
        "@aura/cli/autocompletion/",
        "@aura/cli/autocompletion/fuzzy.py",
    ]
